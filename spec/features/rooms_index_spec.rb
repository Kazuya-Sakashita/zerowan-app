  ...
  describe 'ページネーションのテスト' do
    ...
    let!(:rooms) do
      (1..11).map do |i|
        ...
        room = create(:room, user: joined_users.first, owner: owned_user, pet: pet)
        create(:message, room: room, user: joined_users.first, body: "テストメッセージ#{i}", created_at: Time.current + i.minutes)
        room
      end
    end
